...
def get_active_sessions():
    """
    현재 KST 기준으로 활성화된 트레이딩 세션을 판단하여 리스트로 반환합니다.
    - 늦은 미국/아시아 전 (00:00–08:00): 유동성 낮고 가짜 돌파 주의
    - 아시아 세션 (08:00–16:00): 중간 변동성, 추세 유지 가능
    - 런던 세션 (16:00–00:00): 하루 첫 큰 움직임, 추세 설정
    - 미국 세션 (22:30–06:00): 강한 변동성과 경제지표 발표 구간 포함
    """
    ...
    # 늦은 미국/아시아 전: 00:00 ~ 08:00
    if 0 <= hour < 8:
        sessions.append("Late US/Asia Pre (00:00-08:00)")

    # 아시아 세션: 08:00 ~ 16:00
    if 8 <= hour < 16:
        sessions.append("Asia Session (08:00-16:00)")

    # 런던 세션: 16:00 ~ 00:00
    if 16 <= hour < 24:
        sessions.append("London Session (16:00-00:00)")

    # 미국 세션: 22:30 ~ 06:00 (자정 넘어 계산)
    if (hour > 22) or (hour == 22 and minute >= 30) or (hour < 6):
        sessions.append("US Session (22:30-06:00)")
        # 미국 경제지표 발표 구간: 22:30 ~ 23:30
        if (hour == 22 and minute >= 30) or (hour == 23 and minute < 30):
            sessions.append("US Economic News (22:30-23:30)")
    return sessions
...
def generate_gpt_prompt(wallet_balance, position_info, aggregated_data, extended_data, onchain_data, multi_tf_data,
                        market_regime, thresholds, heatmap_analysis, active_sessions):
    """
    ...
    """
    trading_session_info = ", ".join(active_sessions)
    # 기존 multi_tf_summary 등은 그대로 유지
    multi_tf_summary = ""
    for tf, data in multi_tf_data.items():
        multi_tf_summary += (
    ...
    prompt = f"""
    <TradeBotPrompt>
        <TradingSession>{trading_session_info}</TradingSession>
        <Account>
    ...
    {multi_tf_summary}
        ...
        <DecisionRules>
            ...
            <StrategySwitch>
                <Guide>
                    Based on the market regime and current trading sessions (e.g. false breakout risk during Late US/Asia Pre or high volatility during US Economic News),
                    adopt a trend-following strategy in bullish conditions or a mean reversion approach in sideways or bearish conditions.
                    Incorporate technical, on-chain, sentiment, and liquidation heatmap data into your decision.
                </Guide>
            </StrategySwitch>
        </DecisionRules>
        <Task>
            Based on the provided account information, market context, indicators (technical and on-chain), liquidation heatmap analysis, current trading sessions, and dynamic decision rules, decide whether to GO LONG, GO SHORT, HOLD LONG, HOLD SHORT, or NO TRADE.
...
def generate_trading_decision(wallet_balance, position_info, aggregated_data, extended_data, onchain_data,
                              multi_tf_data, market_regime, thresholds, heatmap_analysis, active_sessions):
    ...
    developer_prompt = (
        ...
        "Based on market regime, current trading sessions and all provided data including liquidation heatmap analysis, "
        "adopt either a trend-following or mean reversion strategy accordingly."
    )
...
def main():
    ...
    logging.info(f"Active trading sessions: {active_sessions}")

    # 6) 뉴스 발표 구간 체크: 미국 경제지표 발표(22:30–23:30) 동안 신규 진입 금지
    if "US Economic News (22:30-23:30)" in active_sessions and not in_position:
        logging.info("US Economic News period active: 신규 포지션 진입을 자제합니다.")
        return

    # 7) 명확한 신호 없으면 GPT 호출하지 않음

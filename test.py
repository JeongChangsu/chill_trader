...
def fetch_high_leverage_ratio():
    """
    무료 데이터 소스(예: Coinglass)에서 고레버리지 포지션 비율 데이터를 크롤링하여 반환.
    (실제 URL 및 셀렉터는 대상 사이트 구조에 맞게 수정 필요)
    """
    url = "https://www.coinglass.com/ko"  # 예시: 메인 페이지 또는 고레버리지 관련 페이지
    try:
        driver = get_driver('high_leverage')
        driver.get(url)
        time.sleep(3)  # 페이지 로딩 대기
        # 예시: 고레버리지 비율이 담긴 요소의 XPath (실제 셀렉터는 사이트 구조에 따라 달라질 수 있음)
        ratio_text = driver.find_element(By.XPATH, '//span[@class="leverage-ratio"]').text
        return ratio_text
    except Exception as e:
        print(f"Error fetching high leverage ratio: {e}")
        return "N/A"


def fetch_liquidation_map():
    """
    무료 데이터 소스(예: Coinglass의 청산맵 페이지)에서 청산맵 정보를 크롤링하여 반환.
    (실제 URL 및 셀렉터는 대상 사이트 구조에 맞게 수정 필요)
    반환 예시: 특정 가격대와 해당 구간의 청산량 정보를 포함한 문자열.
    """
    url = "https://www.coinglass.com/ko/pro/futures/LiquidationMap"
    try:
        driver = get_driver('liquidation_map')
        driver.get(url)
        time.sleep(5)  # 동적 컨텐츠 로딩 대기
        # 예시: 청산맵에서 각 가격대별 청산량 정보가 포함된 요소들 추출 (실제 XPath는 사이트에 맞게 수정)
        liq_elements = driver.find_elements(By.XPATH, '//div[contains(@class, "liq-bar")]')
        liquidation_data = []
        for elem in liq_elements:
            price = elem.get_attribute("data-price")
            volume = elem.get_attribute("data-volume")
            if price and volume:
                liquidation_data.append(f"Price {price}: Volume {volume}")
        return "; ".join(liquidation_data) if liquidation_data else "N/A"
    except Exception as e:
        print(f"Error fetching liquidation map data: {e}")
        return "N/A"
...
{multi_tf_summary}
        ...
        <AdditionalData>
            ...
            <LiquidationMap>{extended_data.get('liquidation_map', 'N/A')}</LiquidationMap>
        </AdditionalData>
    </MarketContext>
    <Indicators>
        <RSI>
        ...
        <GoogleTrends>
            <Guide>
                Peak search interest (near 100) signals extreme public attention and potential market turning points.
            </Guide>
        </GoogleTrends>
        <FearAndGreedIndex>
            <Guide>
        ...
        <HighLeverageRatio>
            <Guide>
                A high leverage ratio (extracted from Coinglass) indicates many over-leveraged positions.
                Adjust position sizing and risk management accordingly, particularly during volatile market conditions.
            </Guide>
        </HighLeverageRatio>
        <LiquidationMap>
            <Guide>
                The liquidation map shows key price levels with significant forced liquidations.
                Identify specific price ranges with aggregated liquidation volumes ("magnet zones") that can trigger rapid moves.
                Use this data to refine entry and exit strategies.
            </Guide>
        </LiquidationMap>
    </Indicators>
    <DecisionRules>

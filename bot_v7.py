...
def analyze_liquidation_map_gemini(image_path):
    """
    ...
    try:
        prompt_parts = [
            image,
            """
Analyze the provided liquidation map for [Asset, e.g., BTC/USDT].  This map shows the estimated liquidation levels of long and short positions.  Your analysis should be extremely detailed and precise, focusing on actionable insights for trading.  Consider *all* visual elements (colors, density, gradients, peaks, gaps, etc.) and their implications. Do NOT give generic advice; base your answers ONLY on the data visible in this image.

1. **Liquidation Level Identification (EXTREMELY DETAILED):**

    *   **Long Liquidation Zones (BELOW Current Price):**
        *   **Precise Price Ranges:**  Identify *all* significant price ranges where long liquidations are concentrated.  Be as specific as possible with the price levels (e.g., "$45,120 - $45,200", *not* "around $45,000").  Prioritize the most significant zones (highest density/volume).
        *   **Relative Size:** For *each* identified price range, describe the *relative* size of the liquidation volume compared to other zones (e.g., "moderate", "large", "very large", "the largest concentration of long liquidations below the current price").  Use consistent terminology.
        *   **Absolute Size (If Possible):** If the chart provides any indication of absolute liquidation volume (e.g., dollar value, number of contracts), estimate the *absolute* size for each key range (e.g., "approximately $50 million", "estimated 1,000 BTC").
        *   **Density Gradient:** Describe how the density of liquidations changes *within* each zone.  Does it increase gradually, sharply, or is it uniform? (e.g., "Liquidations increase sharply at the lower end of the range, around $45,120").
        *   **Gaps:** Identify any significant *gaps* or areas with *low* liquidation volume between the major zones.  Specify the price ranges of these gaps.
        *   **Stair-Step Pattern:** Explicitly state if the long liquidation levels form a "stair-step" pattern (a series of increasingly large liquidation clusters at progressively lower prices). This indicates potential cascading liquidations.

    *   **Short Liquidation Zones (ABOVE Current Price):**
        *   Repeat all the steps above (Precise Price Ranges, Relative Size, Absolute Size, Density Gradient, Gaps, Stair-Step Pattern) for short liquidation zones.

2. **Price Movement Prediction (BASED ON LIQUIDATION DATA ONLY):**

    *   **Scenario Analysis (Price Increase):**
        *   "If the price rises to [Price Level 1], what is the *likely* consequence based on the liquidation map?  (e.g., 'Minor short liquidations, likely causing a small price increase', 'Significant short liquidations, potentially triggering a short squeeze and a sharp price increase')."
        *   Repeat for multiple key price levels, moving upwards.  Focus on levels where liquidations are concentrated.
        *   "If the price reaches [Price Level X] and triggers significant short liquidations, what is the *next* likely resistance level based on the liquidation map?"

    *   **Scenario Analysis (Price Decrease):**
        *   "If the price falls to [Price Level 1], what is the *likely* consequence based on the liquidation map?"
        *   Repeat for multiple key price levels, moving downwards.
        *    "If the price reaches [Price Level Y] and triggers significant long liquidations, what is the *next* likely support level based on the liquidation map?"
    *  **Volatility:**
        *   "Based on the *spacing* and *density* of liquidation levels, is the market likely to experience high or low volatility in the short term?" Explain your reasoning. (e.g., "Closely spaced, high-density liquidation levels suggest high potential volatility.")
        * "Are there price ranges which are likely to see rapid price movement?"
        * "Are there price ranges which are likely to see slow/consolidating movement?"

3. **Risk Assessment:**

    *   **Long vs. Short Risk:**  "Based on the overall distribution of liquidations, do longs or shorts face a greater risk of liquidation in the current market?"  Provide *specific* evidence from the map to support your conclusion (e.g., "Longs face greater risk due to the large, dense clusters of liquidations between $X and $Y").
    *   **High-Risk Zones:** "Identify the *most* dangerous price ranges for *both* longs and shorts.  These are the areas where the largest and most rapid price moves are likely to occur."

4. **(Optional - Advanced) Trading Strategy Ideas (ONLY IF CONFIDENT):**
   * "Based on the liquidation map *alone*, suggest potential trade entry and exit points. Explain your reasoning." Include ALL of the following:
      - Proposed trade direction (LONG or SHORT)
      - Entry price (or price range)
      - Stop Loss price
      - Take Profit price
      - Recommended leverage level (with justification based on the map, e.g. "Low leverage due to high volatility indicated.")
      - Rationale (EXPLAIN IN DETAIL WHY. What specific features of this liquidation map justify your trade idea?)

**Important Notes for Gemini:**

*   **Focus on the Image:**  Base your *entire* analysis on the provided liquidation map image. Do *not* speculate or use external information.
*   **Be Specific:**  Avoid vague terms. Use precise price levels and descriptive language.
*   **Prioritize:**  Focus on the most significant liquidation levels and their potential impact.
*   **Explain Reasoning:**  Clearly explain the *why* behind your conclusions.
* **Output Format (Comma-Separated):**
    *   For part 1, separate sections for longs and shorts. Within each, list price ranges and descriptions, separated by semicolons. Example: "Longs: $45,120-$45,200: Large, increasing density; $44,800-$44,900: Moderate, uniform density; Gap: $45,200-$45,500. Shorts: ..."
    * For Part 2 and 3: Use concise sentences and clear language.
    * For Part 4: If provided, use the format: "Direction, Entry, Stop Loss, Take Profit, Leverage, Rationale". Example: "LONG, $45200, $44900, $46000, 3x, Large long liquidation cluster below entry provides support; smaller short cluster above provides target."

**Example Output (Illustrative - Depends on the Image):**
Longs: $92,340-$92,830: Moderate, evenly distributed; $91,850-$92,000: Very large, increasing density towards $91,850; Gap: $92,000-$92,340. Shorts: $96,750-$97,240: Large, dense cluster; $98,220-$98,710: Moderate, decreasing density; $99,690-$100,180: Small, evenly distributed; Gap: $97,240-$98,220.
"""]
...
def generate_trading_decision(extended_data,
    ...
    """
    prompt = generate_gemini_prompt(extended_data,
                                    onchain_data, multi_tf_data, market_regime, thresholds,
                                    heatmap_analysis, econ_summary, primary_tf, current_session,
                                    fake_breakout_info, session_volatility_info)
...
def log_closed_position(symbol, entry_price, exit_price, trade_side, leverage, tp_price, sl_price):
    """
    청산된 포지션의 내역 및 수익을 CSV 파일에 기록한다. (수정)
    """
    profit = (exit_price - entry_price) * leverage if trade_side.upper() == "LONG" else (
    ...
    with open(CLOSED_POSITIONS_FILE, mode='a', newline='') as file:
        ...
        if not file_exists:
            writer.writerow(
                ["timestamp", "symbol", "trade_side", "entry_price", "exit_price", "profit", "profit_percent",
                 "is_win", 'leverage', 'tp_price', 'sl_price'])
        writer.writerow(
            [datetime.utcnow(), symbol, trade_side, entry_price, exit_price, profit, profit_percent, is_win,
             leverage, tp_price, sl_price])
    logging.info(f"{symbol} closed position logged (profit: {profit:.2f}, {profit_percent:.2f}%).")
...
def create_hyperliquid_order(symbol, decision, leverage, entry_price):
    ...
    try:
        ...
        params = {
            'takeProfit': tp_price,  # Take Profit
            'stopLoss': sl_price,  # Stop Loss
            'test': False  # 실거래
        }
        ...
        logging.info(f"Hyperliquid order created: {order}")
        return order
    except Exception as e:
...
def main():
    ...
    try:
        ...
        heatmap_analysis = analyze_liquidation_map_gemini(
            "/Users/changpt/Downloads/Liquidation Map.png")
    ...
    if decision['final_action'] in ['GO LONG', 'GO SHORT']:
        ...
        order = create_hyperliquid_order(SYMBOL, decision, float(decision['leverage'].replace('x', '')), cprice)
